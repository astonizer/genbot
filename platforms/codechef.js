...
let fetchProblemsListFromCodechefContest = (html) => {
    const $ = cheerio.load(html);
    console.log("Parsing problems from contest");
    console.log('jkl', $('a[class=ember-view]'));
    $('a[class=ember-view]').each((id, problem) => {
        // let problem_url = 'https://www.codechef.com' + $(problem).attr('href');
        // console.log(problem_url);
        // fetchAllProblemsOfContest(problem_url);
        console.log($(problem).attr('href'));
    });
    // console.log($('tr.ember-view td div span a'));
};

module.exports = fetchProblemsListFromCodechefContest;